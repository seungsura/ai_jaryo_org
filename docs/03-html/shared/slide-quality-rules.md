...
- 한국어 문구 품질 작업에는 `.codex/skills/local/natural-korean-prose/SKILL.md`를 함께 적용한다. 자연스러운 한국어는 전문 용어를 모두 한국어로 바꾸는 것이 아니라, 정확한 English term을 한국어 어순과 호흡 안에 놓는 것이다.
...
- 모든 slide를 1-2줄 statement page로 압축하지 않는다. 숫자, 근거, 구조, 비교, process, architecture를 보여줘야 하는 slide는 `metric card trio`, `evidence cards without tables`, `evidence table`, `process`, `native diagram`, `split comparison` 같은 layout family를 우선 검토한다.
- slide는 짧아야 한다가 아니라 역할이 선명해야 한다. statement slide는 한 문장 중심으로 두고, evidence/metric slide는 source-backed 근거 묶음이 보이도록 설계한다.
...
- 2026-04-23 current-main S024 relation remap: 사용자는 24페이지에서 `파일 읽기 -> 셸 실행 -> 테스트 결과 -> 다시 수정` 루프를 제거하라고 지시했다. 24페이지는 `하네스가 해내야 하는 것`과 `하네스를 이루는 것들`의 관계를 보여주는 relation-map 계열 family로 다시 할당한다. 왼쪽은 `무엇을 보는지/할 수 있는지/언제 멈추는지/잘못되었을 때` 4개 responsibility card, 오른쪽은 `Plan Mode · 승인 체계`, `CLAUDE.md · Skills · Hooks`, `MCP · Plugins · Subagents` 3개 group card로 압축한다. 관계성은 좌우 사이 연결선과 공간 배치로 드러내고, 별도의 formula/equation/loop/process strip를 다시 넣지 않는다.
...
- 2026-04-23 agent-loop reuse note: 사용자는 `에이전트 루프` HTML artifact를 나중에 재사용할 수 있도록 workspace root에 복사해 두라고 지시했다. 향후 CHAPTER 04/07 관련 피드백에서는 generic `에이전트 루프`를 독립 loop grammar로 유지하되, `Ralph Loop`는 그 loop의 구체 예시 또는 반복 자산화 장면으로 연결해 검토한다. 두 slide를 하나로 섞거나 source 밖 비교 축을 새로 만들지 않는다.
