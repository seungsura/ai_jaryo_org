# Harness Rebuilt Manifest

- slide count: 127
- output deck: `/Users/seungsu/Desktop/project/jaryo/docs/03-html/harness-rebuilt/deck/index.html`
- active theme: `theme-minimal-light`

## Chapter Files

- chapter `00` brief: `briefs/chapter-00.md`
- chapter `00` copy: `copy/chapter-00.md`
- chapter `01` brief: `briefs/chapter-01.md`
- chapter `01` copy: `copy/chapter-01.md`
- chapter `02` brief: `briefs/chapter-02.md`
- chapter `02` copy: `copy/chapter-02.md`
- chapter `03` brief: `briefs/chapter-03.md`
- chapter `03` copy: `copy/chapter-03.md`
- chapter `04` brief: `briefs/chapter-04.md`
- chapter `04` copy: `copy/chapter-04.md`
- chapter `05` brief: `briefs/chapter-05.md`
- chapter `05` copy: `copy/chapter-05.md`
- chapter `06` brief: `briefs/chapter-06.md`
- chapter `06` copy: `copy/chapter-06.md`

## Slide Registry

| order | slide id | kind | shell | chapter | source block | file | title | footnote assets |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `S001` | `cover` | `cover-title-shell` | `00` | `00-01` | `slide-001.html` | Harness를 설계하는 법 | - |
| 2 | `S002` | `content` | `content-emphasis-shell` | `00` | `00-01` | `slide-002.html` | Harness를 설계하는 법 | - |
| 3 | `S003` | `content` | `content-essay-shell` | `00` | `00-02` | `slide-003.html` | 시작하며 | - |
| 4 | `S004` | `content` | `content-essay-shell` | `00` | `00-02` | `slide-004.html` | AI 를 사용해본 경험은 다들 있으실 것 | - |
| 5 | `S005` | `content` | `content-essay-shell` | `00` | `00-03` | `slide-005.html` | 코딩보다 환경 구성을 해야 | - |
| 6 | `S006` | `content` | `content-essay-shell` | `00` | `00-04` | `slide-006.html` | 앞으로 다룰 이야기 | - |
| 7 | `S007` | `content` | `content-card-shell` | `00` | `00-04` | `slide-007.html` | 이 글은 특정 제품의 도구 리뷰가 아닙니다 | - |
| 8 | `S008` | `content` | `content-emphasis-shell` | `00` | `00-04` | `slide-008.html` | 앞으로 다룰 이야기 | - |
| 9 | `S009` | `chapter-transition` | `chapter-transition-shell` | `01` | `01-01` | `slide-009.html` | 코딩은 사라지는가 | - |
| 10 | `S010` | `content` | `content-essay-shell` | `01` | `01-01` | `slide-010.html` | 직접 붙들던 층은 계속 위로 밀려났다 | - |
| 11 | `S011` | `content` | `content-card-shell` | `01` | `01-01` | `slide-011.html` | 반발의 문장은 시대만 바뀌고 반복됐다 | - |
| 12 | `S012` | `content` | `content-essay-shell` | `01` | `01-01` | `slide-012.html` | 코드 예시는 역할 이동을 가장 빨리 보여 준다 | - |
| 13 | `S013` | `content` | `content-essay-shell` | `01` | `01-01` | `slide-013.html` | AI도 예외가 아니라 같은 흐름의 다음 장면 | - |
| 14 | `S014` | `content` | `content-emphasis-shell` | `01` | `01-01` | `slide-014.html` | 타이핑보다 설계와 검증이 더 무거워진다 | - |
| 15 | `S015` | `content` | `content-essay-shell` | `01` | `01-02` | `slide-015.html` | 추상화의 역사: 직접 하던 일은 계속 줄어들었다 | page-003.jpg |
| 16 | `S016` | `content` | `content-card-shell` | `01` | `01-02` | `slide-016.html` | AI 코딩 도구가 빠르게 좋아질수록 사람들은 같은 질문을 반복 | page-003.jpg |
| 17 | `S017` | `content` | `content-essay-shell` | `01` | `01-03` | `slide-017.html` | 숫자로 보는 변화 | - |
| 18 | `S018` | `content` | `content-essay-shell` | `01` | `01-04` | `slide-018.html` | 문서가 코드다 | - |
| 19 | `S019` | `content` | `content-essay-shell` | `01` | `01-05` | `slide-019.html` | 개발자의 새로운 역할: 컨텍스트를 설계하는 능력 | - |
| 20 | `S020` | `content` | `content-card-shell` | `01` | `01-05` | `slide-020.html` | 과거의 개발자는 코드를 잘 작성하고 버그를 잘 고치는 사람이었 | - |
| 21 | `S021` | `content` | `content-emphasis-shell` | `01` | `01-05` | `slide-021.html` | 개발자의 새로운 역할: 컨텍스트를 설계하는 능력 | - |
| 22 | `S022` | `content` | `content-essay-shell` | `01` | `01-06` | `slide-022.html` | 그래도 기초가 중요하다 | - |
| 23 | `S023` | `content` | `content-essay-shell` | `01` | `01-06` | `slide-023.html` | AI가 코드를 더 많이 써 줄수록 기본기가 덜 중요해질 것처럼 보 | - |
| 24 | `S024` | `chapter-transition` | `chapter-transition-shell` | `02` | `02-01` | `slide-024.html` | 왜 Claude Code인가, 그리고 왜 Harness 인가 | - |
| 25 | `S025` | `content` | `content-essay-shell` | `02` | `02-01` | `slide-025.html` | 왜 Claude Code인가, 그리고 왜 Harness 인가 | - |
| 26 | `S026` | `content` | `content-essay-shell` | `02` | `02-01` | `slide-026.html` | 시작하며: 특정 제품 리뷰가 아닙니다 | - |
| 27 | `S027` | `content` | `content-essay-shell` | `02` | `02-02` | `slide-027.html` | 막: Copilot과 ChatGPT, 프롬프트의 시대 | 02-chain-of-thought.png, 03-react-pattern.png, 04-tree-of-thought.png, 05-andrew-ng-agentic-design-patterns.png |
| 28 | `S028` | `content` | `content-card-shell` | `02` | `02-02` | `slide-028.html` | AI 코딩 도구의 출발점에는 GitHub Copilot이 있었 | 02-chain-of-thought.png, 03-react-pattern.png, 04-tree-of-thought.png, 05-andrew-ng-agentic-design-patterns.png |
| 29 | `S029` | `content` | `content-essay-shell` | `02` | `02-02` | `slide-029.html` | 초창기 Copilot은 현재 파일의 코드를 바탕으로 다음 줄을 제안하는 스마트 자동완성에 가까웠 | 02-chain-of-thought.png, 03-react-pattern.png, 04-tree-of-thought.png, 05-andrew-ng-agentic-design-patterns.png |
| 30 | `S030` | `content` | `content-emphasis-shell` | `02` | `02-02` | `slide-030.html` | 막: Copilot과 ChatGPT, 프롬프트의 시대 | 02-chain-of-thought.png, 03-react-pattern.png, 04-tree-of-thought.png, 05-andrew-ng-agentic-design-patterns.png |
| 31 | `S031` | `content` | `content-essay-shell` | `02` | `02-03` | `slide-031.html` | 프롬프트 시대의 벽: 모델은 보지 못한 것을 알 수 | - |
| 32 | `S032` | `content` | `content-essay-shell` | `02` | `02-04` | `slide-032.html` | 막: Cursor와 컨텍스트의 시대 | 06-cursor-ai-code-editor-architecture.png |
| 33 | `S033` | `content` | `content-card-shell` | `02` | `02-04` | `slide-033.html` | 프롬프트의 벽을 가장 먼저 크게 밀어낸 도구가 Cursor였 | 06-cursor-ai-code-editor-architecture.png |
| 34 | `S034` | `content` | `content-emphasis-shell` | `02` | `02-04` | `slide-034.html` | 막: Cursor와 컨텍스트의 시대 | 06-cursor-ai-code-editor-architecture.png |
| 35 | `S035` | `content` | `content-essay-shell` | `02` | `02-05` | `slide-035.html` | 컨텍스트 시대의 벽: 좋은 입력만으로는 루프를 통제할 수 | - |
| 36 | `S036` | `content` | `content-card-shell` | `02` | `02-05` | `slide-036.html` | 하지만 컨텍스트만으로도 충분하지 않았 | - |
| 37 | `S037` | `content` | `content-emphasis-shell` | `02` | `02-05` | `slide-037.html` | 컨텍스트 시대의 벽: 좋은 입력만으로는 루프를 통제할 수 | - |
| 38 | `S038` | `content` | `content-essay-shell` | `02` | `02-06` | `slide-038.html` | 막: Claude Code, Codex, OpenCode 와 하네스의 시대 | 01-three-era-timeline.png, page-002.jpg |
| 39 | `S039` | `content` | `content-card-shell` | `02` | `02-06` | `slide-039.html` | 하네스 시대의 대표 장면은 코딩 도구가 단순한 자동완성이나 채팅을 넘어 작업 환경 전체를 품기 시작한 것 | 01-three-era-timeline.png, page-002.jpg |
| 40 | `S040` | `content` | `content-emphasis-shell` | `02` | `02-06` | `slide-040.html` | 막: Claude Code, Codex, OpenCode 와 하네스의 시대 | 01-three-era-timeline.png, page-002.jpg |
| 41 | `S041` | `chapter-transition` | `chapter-transition-shell` | `03` | `03-01` | `slide-041.html` | AI 시대의 개발 방법론 | - |
| 42 | `S042` | `content` | `content-essay-shell` | `03` | `03-01` | `slide-042.html` | AI 시대의 개발 방법론 | page-026.png |
| 43 | `S043` | `content` | `content-card-shell` | `03` | `03-01` | `slide-043.html` | 왜 지금 방법론 | page-026.png |
| 44 | `S044` | `content` | `content-essay-shell` | `03` | `03-02` | `slide-044.html` | TDD: AI 가드레일 | - |
| 45 | `S045` | `content` | `content-card-shell` | `03` | `03-02` | `slide-045.html` | 에이전트는 "그럴듯하게 작동하는 코드"를 너무 쉽게, 그리고 너무 자신 있게 내놓 | - |
| 46 | `S046` | `content` | `content-emphasis-shell` | `03` | `03-02` | `slide-046.html` | TDD: AI 가드레일 | - |
| 47 | `S047` | `content` | `content-essay-shell` | `03` | `03-03` | `slide-047.html` | SDD: 스펙이 진실의 원천 | - |
| 48 | `S048` | `content` | `content-card-shell` | `03` | `03-03` | `slide-048.html` | 구현 단계에서 TDD가 에이전트의 일탈을 막아낸다면, 그 이전 단계에서 방향을 통제하는 것은 SDD | - |
| 49 | `S049` | `content` | `content-essay-shell` | `03` | `03-04` | `slide-049.html` | Waterfall과는 무엇이 다른가 | page-032.png |
| 50 | `S050` | `content` | `content-card-shell` | `03` | `03-04` | `slide-050.html` | 이쯤 되면 당연한 반론이 나옵니다 | page-032.png |
| 51 | `S051` | `content` | `content-emphasis-shell` | `03` | `03-04` | `slide-051.html` | Waterfall과는 무엇이 다른가 | page-032.png |
| 52 | `S052` | `chapter-transition` | `chapter-transition-shell` | `04` | `04-01` | `slide-052.html` | 프롬프트를 넘어서: 에이전트를 움직이는 기술, Harness | - |
| 53 | `S053` | `content` | `content-essay-shell` | `04` | `04-01` | `slide-053.html` | 프롬프트를 넘어서: 에이전트를 움직이는 기술, Harness | - |
| 54 | `S054` | `content` | `content-essay-shell` | `04` | `04-02` | `slide-054.html` | Prompt, Context, Harness | page-002.jpg |
| 55 | `S055` | `content` | `content-card-shell` | `04` | `04-02` | `slide-055.html` | 셋은 비슷한 유행어가 아니라 서로 다른 문제를 다룹니다 | page-002.jpg |
| 56 | `S056` | `content` | `content-emphasis-shell` | `04` | `04-02` | `slide-056.html` | Prompt, Context, Harness | page-002.jpg |
| 57 | `S057` | `content` | `content-essay-shell` | `04` | `04-03` | `slide-057.html` | Agent = Model + Harness | page-005.jpg |
| 58 | `S058` | `content` | `content-card-shell` | `04` | `04-03` | `slide-058.html` | Agent = Model + Harness라는 말은 과장이 아닙니다 | page-005.jpg |
| 59 | `S059` | `content` | `content-essay-shell` | `04` | `04-04` | `slide-059.html` | Context Engineering: 더 많이 넣는 기술이 아니다 | - |
| 60 | `S060` | `content` | `content-essay-shell` | `04` | `04-04` | `slide-060.html` | 컨텍스트 엔지니어링을 “정보를 많이 넣는 기술”로 이해하면 곧바로 실패 | - |
| 61 | `S061` | `content` | `content-emphasis-shell` | `04` | `04-04` | `slide-061.html` | Context Engineering: 더 많이 넣는 기술이 아니다 | - |
| 62 | `S062` | `content` | `content-card-shell` | `04` | `04-05` | `slide-062.html` | 에이전트 루프: 하네스의 심장 | - |
| 63 | `S063` | `content` | `content-essay-shell` | `04` | `04-06` | `slide-063.html` | Right Altitude: 지시문의 고도 | - |
| 64 | `S064` | `content` | `content-essay-shell` | `04` | `04-07` | `slide-064.html` | 하네스를 만드는 네 가지 기본 도구 | page-043.png |
| 65 | `S065` | `content` | `content-card-shell` | `04` | `04-07` | `slide-065.html` | CLAUDE.md는 시스템 프롬프트라기보다 지속 컨텍스트에 가깝 | page-043.png |
| 66 | `S066` | `content` | `content-essay-shell` | `04` | `04-08` | `slide-066.html` | Stable Prefix와 Variable Suffix: 비용 최적화 | 08-kv-cache-mechanism.png |
| 67 | `S067` | `content` | `content-card-shell` | `04` | `04-08` | `slide-067.html` | 컨텍스트를 잘 구성한다는 말은 단순히 “잘 요약한다”는 뜻이 아닙니다 | 08-kv-cache-mechanism.png |
| 68 | `S068` | `content` | `content-emphasis-shell` | `04` | `04-08` | `slide-068.html` | Stable Prefix와 Variable Suffix: 비용 최적화 | 08-kv-cache-mechanism.png |
| 69 | `S069` | `content` | `content-essay-shell` | `04` | `04-09` | `slide-069.html` | MCP와 Context Hub: 도구와 최신 지식의 연결 | 09-mcp-architecture.png |
| 70 | `S070` | `content` | `content-card-shell` | `04` | `04-09` | `slide-070.html` | 컨텍스트 엔지니어링에서 외부 도구와 최신 문서 연결은 핵심 인프라 | 09-mcp-architecture.png |
| 71 | `S071` | `content` | `content-emphasis-shell` | `04` | `04-09` | `slide-071.html` | MCP와 Context Hub: 도구와 최신 지식의 연결 | 09-mcp-architecture.png |
| 72 | `S072` | `content` | `content-essay-shell` | `04` | `04-10` | `slide-072.html` | Memory: 세션을 넘어서는 기억 | - |
| 73 | `S073` | `content` | `content-card-shell` | `04` | `04-10` | `slide-073.html` | 컨텍스트 윈도우는 작업 기억 | - |
| 74 | `S074` | `content` | `content-emphasis-shell` | `04` | `04-10` | `slide-074.html` | Memory: 세션을 넘어서는 기억 | - |
| 75 | `S075` | `content` | `content-essay-shell` | `04` | `04-11` | `slide-075.html` | 기능 블록과 도구 레이어는 다르다 | - |
| 76 | `S076` | `content` | `content-card-shell` | `04` | `04-11` | `slide-076.html` | 하네스를 설계할 때 가장 많이 생기는 혼동은 기능 블록과 도구 레이어를 섞는 것 | - |
| 77 | `S077` | `content` | `content-essay-shell` | `04` | `04-12` | `slide-077.html` | CLAUDE.md와 AGENTS.md: 규칙을 정 | - |
| 78 | `S078` | `content` | `content-card-shell` | `04` | `04-12` | `slide-078.html` | CLAUDE.md나 AGENTS.md 같은 규칙 파일은 프로젝트의 기준선을 고정 | - |
| 79 | `S079` | `content` | `content-essay-shell` | `04` | `04-13` | `slide-079.html` | Skills, Hooks, MCP는 같은 말이 아니다 | - |
| 80 | `S080` | `content` | `content-essay-shell` | `04` | `04-14` | `slide-080.html` | Plan-Critic-Build: 계획과 실행을 갈라놓기 | - |
| 81 | `S081` | `content` | `content-essay-shell` | `04` | `04-15` | `slide-081.html` | Ralph Loop: 가장 단순한 하네스 | - |
| 82 | `S082` | `content` | `content-essay-shell` | `04` | `04-16` | `slide-082.html` | 암묵지를 파일로 뽑아내라 | - |
| 83 | `S083` | `content` | `content-essay-shell` | `04` | `04-17` | `slide-083.html` | Harness Builder라는 역할 | - |
| 84 | `S084` | `content` | `content-essay-shell` | `04` | `04-18` | `slide-084.html` | LLM-as-OS: 프롬프트는 명령어 한 줄일 뿐 | - |
| 85 | `S085` | `content` | `content-card-shell` | `04` | `04-18` | `slide-085.html` | 기존 OS 구성요소 / 역할 / LLM 시스템 대응 | - |
| 86 | `S086` | `content` | `content-essay-shell` | `04` | `04-19` | `slide-086.html` | 하네스 엔지니어링의 5대 기능 블록 | page-005.jpg |
| 87 | `S087` | `content` | `content-card-shell` | `04` | `04-19` | `slide-087.html` | 앞서 본 여섯 개 실행 요소를 운영 관점으로 다시 묶으면 다섯 개 기능 블록으로 정리할 수 있 | page-005.jpg |
| 88 | `S088` | `content` | `content-emphasis-shell` | `04` | `04-19` | `slide-088.html` | 하네스 엔지니어링의 5대 기능 블록 | page-005.jpg |
| 89 | `S089` | `chapter-transition` | `chapter-transition-shell` | `05` | `05-01` | `slide-089.html` | 이렇게 하면 망한다: 한계와 실패 패턴 | - |
| 90 | `S090` | `content` | `content-essay-shell` | `05` | `05-01` | `slide-090.html` | 오류의 나비효과 | - |
| 91 | `S091` | `content` | `content-essay-shell` | `05` | `05-02` | `slide-091.html` | 긴 작업 사슬이 특히 위험한 이유 | - |
| 92 | `S092` | `content` | `content-essay-shell` | `05` | `05-03` | `slide-092.html` | 컨텍스트가 길수록 항상 좋은 것은 아니다 | - |
| 93 | `S093` | `content` | `content-essay-shell` | `05` | `05-04` | `slide-093.html` | 대표 실패 패턴 네 가지 | - |
| 94 | `S094` | `content` | `content-essay-shell` | `05` | `05-04` | `slide-094.html` | 긴 세션에서 반복해서 나타나는 실패는 4가지 정도로 분류하고 있 | - |
| 95 | `S095` | `content` | `content-essay-shell` | `05` | `05-05` | `slide-095.html` | 현실에서 보이는 증상들 | - |
| 96 | `S096` | `content` | `content-essay-shell` | `05` | `05-05` | `slide-096.html` | AI Slop은 산출물이 급격히 조잡해지는 상태를 가리킵니다 | - |
| 97 | `S097` | `content` | `content-essay-shell` | `05` | `05-06` | `slide-097.html` | Context Rot: 길어진 기억은 조용히 썩는다 | - |
| 98 | `S098` | `content` | `content-essay-shell` | `05` | `05-07` | `slide-098.html` | 신뢰는 조율되어야 | - |
| 99 | `S099` | `content` | `content-essay-shell` | `05` | `05-08` | `slide-099.html` | 결정 제어와 확률 제어를 분리하라 | page-009.jpg, page-010.jpg, page-011.jpg |
| 100 | `S100` | `content` | `content-card-shell` | `05` | `05-08` | `slide-100.html` | 하네스 설계에서 중요한 원칙은 기계가 확실히 판단할 수 있는 것과 AI가 유연하게 판단해야 하는 것을 분리하는 일 | page-009.jpg, page-010.jpg, page-011.jpg |
| 101 | `S101` | `content` | `content-emphasis-shell` | `05` | `05-08` | `slide-101.html` | 결정 제어와 확률 제어를 분리하라 | page-009.jpg, page-010.jpg, page-011.jpg |
| 102 | `S102` | `content` | `content-essay-shell` | `05` | `05-09` | `slide-102.html` | SOLID도 하네스 관점으로 | page-012.jpg |
| 103 | `S103` | `content` | `content-card-shell` | `05` | `05-09` | `slide-103.html` | 하네스는 새로운 유행어지만, 기존 소프트웨어 원칙을 버리라는 뜻은 아닙니다 | page-012.jpg |
| 104 | `S104` | `chapter-transition` | `chapter-transition-shell` | `06` | `06-01` | `slide-104.html` | 멀티 에이전트 활용 패턴 | - |
| 105 | `S105` | `content` | `content-essay-shell` | `06` | `06-01` | `slide-105.html` | 시작하며: 병렬화보다 분해가 먼저다 | - |
| 106 | `S106` | `content` | `content-essay-shell` | `06` | `06-01` | `slide-106.html` | 멀티 에이전트는 AI를 더 많이 붙이는 묘기가 아닙니다 | - |
| 107 | `S107` | `content` | `content-essay-shell` | `06` | `06-02` | `slide-107.html` | 왜 하나의 에이전트만으로는 부족한가 | - |
| 108 | `S108` | `content` | `content-essay-shell` | `06` | `06-02` | `slide-108.html` | 하나의 에이전트에게 모든 것을 맡기면 세 가지 문제가 생깁니다 | - |
| 109 | `S109` | `content` | `content-essay-shell` | `06` | `06-03` | `slide-109.html` | Sub-Agent: 중간 작업을 격리하는 기본형 | - |
| 110 | `S110` | `content` | `content-card-shell` | `06` | `06-03` | `slide-110.html` | 가장 자주 쓰는 기본형은 Sub-Agent | - |
| 111 | `S111` | `content` | `content-emphasis-shell` | `06` | `06-03` | `slide-111.html` | Sub-Agent: 중간 작업을 격리하는 기본형 | - |
| 112 | `S112` | `content` | `content-essay-shell` | `06` | `06-04` | `slide-112.html` | Orchestrator: 계획자 하나가 여러 실행자를 배치 | - |
| 113 | `S113` | `content` | `content-card-shell` | `06` | `06-04` | `slide-113.html` | Orchestrator는 큰 목표를 하위 작업으로 나누고 여러 실행자에게 배치하는 패턴 | - |
| 114 | `S114` | `content` | `content-essay-shell` | `06` | `06-05` | `slide-114.html` | Parallel: 같은 목표를 평면으로 벌리고 나중에 합친다 | - |
| 115 | `S115` | `content` | `content-card-shell` | `06` | `06-05` | `slide-115.html` | Parallel은 이름 그대로 여러 작업을 동시에 돌리는 패턴 | - |
| 116 | `S116` | `content` | `content-essay-shell` | `06` | `06-06` | `slide-116.html` | GAN-Style: 생성자와 평가자를 분리 | - |
| 117 | `S117` | `content` | `content-card-shell` | `06` | `06-06` | `slide-117.html` | GAN-Style 패턴은 생성과 평가를 같은 주체에 묶지 않는 방식 | - |
| 118 | `S118` | `content` | `content-essay-shell` | `06` | `06-07` | `slide-118.html` | Agent Teams: 양방향 대화가 가능한 팀을 만든다 | - |
| 119 | `S119` | `content` | `content-card-shell` | `06` | `06-07` | `slide-119.html` | Agent Teams는 여러 에이전트가 각자 별도 컨텍스트를 가진 채 직접 메시지를 주고받는 구조 | - |
| 120 | `S120` | `content` | `content-essay-shell` | `06` | `06-08` | `slide-120.html` | Sub-Agent와 Agent Team은 다르다 | page-065.png |
| 121 | `S121` | `content` | `content-card-shell` | `06` | `06-08` | `slide-121.html` | 둘은 자주 혼동되지만 목적이 다릅니다 | page-065.png |
| 122 | `S122` | `content` | `content-essay-shell` | `06` | `06-09` | `slide-122.html` | Advisor 전략: 작은 실행자, 큰 자문 | - |
| 123 | `S123` | `content` | `content-essay-shell` | `06` | `06-10` | `slide-123.html` | 설계 원칙: 패턴보다 경계가 중요하다 | - |
| 124 | `S124` | `content` | `content-essay-shell` | `06` | `06-10` | `slide-124.html` | 패턴 이름보다 중요한 것은 경계 설계 | - |
| 125 | `S125` | `content` | `content-emphasis-shell` | `06` | `06-10` | `slide-125.html` | 설계 원칙: 패턴보다 경계가 중요하다 | - |
| 126 | `S126` | `content` | `content-essay-shell` | `06` | `06-11` | `slide-126.html` | 멀티 모델은 멀티 에이전트와 다르다 | - |
| 127 | `S127` | `content` | `content-emphasis-shell` | `06` | `06-12` | `slide-127.html` | 결론: 더 많은 AI가 아니라 더 좁은 역할의 AI 여럿 | - |
