...
이 부록의 목적은 본문을 다시 요약하는 데 있지 않습니다. 이 글의 주장이 어떤 근거층 위에 서 있는지, 그리고 후속 읽기를 어떤 순서로 이어가야 하는지 분명히 정리하는 데 있습니다. 여기서는 근거를 공개 출처, 내부 확인 정보, 로컬 markdown archive의 보조 자료라는 세 층으로 나눠 봅니다. 이 구분을 분명히 해 두어야 무엇을 공개 1차 근거로 읽고, 무엇을 내부 맥락으로 제한해 읽고, 무엇을 보조 재료로 사용할지 빠르게 판단할 수 있습니다.
...
다음 층은 `카카오 AI 팀` 내부 확인 정보입니다. 본문 일부의 생산성 체감치나 제작 맥락은 이 층에서만 확인되며, 공개 링크로 완전히 대체되지 않습니다. 그래서 본문에서는 이 층을 공개 1차 근거와 같은 무게로 취급하지 않고, 공개 출처 위에 얹히는 제한적 보강 정보로 다룹니다.
...
핵심은 출처의 층위를 섞지 않는 일입니다. 공개 근거는 공개 근거로, 내부 확인 정보는 내부 맥락으로, 로컬 아카이브는 보조 연결망으로 읽어야 합니다. 그래야 본문의 주장도 더 선명해지고, 다음 조사도 덜 흔들립니다.
