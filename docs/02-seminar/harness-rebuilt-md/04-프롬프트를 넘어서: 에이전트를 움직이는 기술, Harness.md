...
차이는 검색 범위와 기준 시점입니다. `RAG`는 넓은 저장소에서 관련 맥락을 찾는 데 강합니다. 반면 `Context Hub`는 지금 기준으로 맞는 문서를 지정해 읽히는 데 강합니다. 내부 위키처럼 넓게 훑어야 하는 영역에서는 `RAG`가 자연스럽고, 최신 API나 SDK처럼 버전과 출처가 중요한 영역에서는 `Context Hub`가 더 안정적입니다.
