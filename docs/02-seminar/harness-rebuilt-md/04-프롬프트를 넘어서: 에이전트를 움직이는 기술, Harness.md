...
| **층위** | **핵심 질문** | **초점** |
| --- | --- | --- |
| `Prompt` | 무엇을/어떻게 말할 것인가 | 명령의 품질 
단일 프롬프트 
정적 |
| `Context` | 무엇을/어떻게 보여줄 것인가 | 정보의 품질 
입력 파이프라인
동적 |
| `Harness` | 무엇을/어떻게 통제할 것인가 | 시스템의 안정성
실행 환경 전체
시스템 아키텍처 |
...
하네스는 프롬프트와 컨텍스트를 없애는 개념이 아닙니다. 둘을 포함하는 구조입니다. 좋은 하네스는 여전히 좋은 프롬프트를 필요로 하고, 좋은 컨텍스트를 필요로 합니다. 다만 그것들이 단발성 요령으로 흩어지지 않게 만듭니다.

셋의 관계를 한 줄로 적으면 `Prompt ⊂ Context ⊂ Harness`입니다. 프롬프트는 한 번 말하는 법이고, 컨텍스트는 그 말이 기대는 정보의 묶음이며, 하네스는 둘이 실제로 돌아가는 실행 환경 전체입니다. 안쪽이 부실하면 바깥도 같이 무너집니다. 모호한 한 줄 프롬프트는 좋은 컨텍스트로도 다 못 메우고, 허술한 컨텍스트는 좋은 하네스 위에서도 계속 잡음을 만듭니다.

## Agent = Model + Harness

`Agent = Model + Harness`라는 말은 과장이 아닙니다. 모델이 텍스트를 생성하는 엔진이라면, 하네스는 그 엔진이 어떤 연료를 먹고 어떤 도구를 잡고 어떤 울타리 안에서 움직일지 정하는 운영체제에 가깝습니다. 그래서 "모델이 아닌 것은 전부 하네스"라는 말이 나옵니다. 하네스는 말의 힘을 수레에 연결하는 마구처럼, 강력하지만 예측 불가능한 LLM을 실제 일에 쓸 수 있게 만드는 장치입니다.

하네스를 실행 관점에서 풀면 여섯 가지 구성 요소로 정리할 수 있습니다.

| **구성 요소** | **무엇을 맡는가** |
| --- | --- |
| `Context Engineering` | 지금 단계에서 모델이 봐야 할 정보와 버려야 할 정보를 고른다 |
| `Tool Orchestration` | 어떤 도구를 어떤 순서와 권한으로 호출할지 정한다 |
| `State & Memory` | 세션이 길어져도 상태가 증발하지 않게 파일과 기록으로 남긴다 |
| `Verification Loop` | 테스트, 린트, 리뷰 같은 결정론적 문을 세운다 |
| `Error Recovery` | 실패를 감지하고 재시도, 우회, 중단 경로를 정한다 |
| `Human-in-the-Loop Control` | 비싼 결정에서 사람 승인과 개입 지점을 고정한다 |

이 여섯 개는 따로 노는 기능이 아닙니다. 결국 하나의 루프를 어디까지 자동화하고 어디서 멈출지 정하는 한 묶음입니다.
...
`CLAUDE.md`는 시스템 프롬프트라기보다 지속 컨텍스트에 가깝습니다. 그래서 더 길기보다 더 짧고, 더 멋지기보다 더 검증 가능해야 합니다. 한 줄씩 지워 봤을 때 지워도 행동이 같다면 그 줄은 규칙이 아니라 잡음일 가능성이 큽니다.

`Skills`는 필요할 때만 꺼내 쓰는 workflow 패키지이고, `Hooks`는 자동으로 실행되는 결정론적 문입니다. `Plugins`는 여기서 한 걸음 더 나가, 개인이 쌓은 `Skill`, `Hook`, `Subagent`, `MCP` 구성을 팀 자산으로 포장하는 배포 레이어입니다. 반복 작업이 `/commit`, `/code-review`, `/kickoff`처럼 한 줄 명령으로 굳는 순간 하네스는 개인 요령이 아니라 팀 운영체제가 됩니다.
...
Manus 사례가 보여준 것도 바로 이것입니다. 프롬프트를 더 영리하게 쓰는 능력보다, 접두어를 함부로 흔들지 않는 능력이 프로덕션 비용과 지연을 더 크게 좌우합니다. 시스템 프롬프트, 도구 정의, 장기 규칙은 최대한 고정하고, 최신 입력과 도구 결과만 뒤에서 갈아 끼워야 `KV-cache hit rate`가 살아납니다. 하네스 시대에는 잘 쓰는 것 못지않게 안 바꾸는 것도 능력입니다.

```text
Stable Prefix
시스템 프롬프트 / 도구 정의 / 장기 규칙 / 스킬 목록
   ↓
같게 유지될수록 KV-cache 재사용
   ↓
비용과 지연 감소
...
핵심은 새 기능이 아닙니다. 연결 방식을 표준화한다는 점입니다. 도구가 늘어도 호출 방식과 결과 형식이 크게 흔들리지 않아야, 에이전트도 같은 리듬으로 읽고 판단할 수 있습니다.
...
| **구분** | `RAG` | `Context Hub` |
| --- | --- | --- |
| 무엇을 가져오나 | 문서 저장소에서 관련 청크를 찾는다 | 지정된 최신 문서를 바로 가져온다 |
| 언제 강한가 | 내부 위키, 정책 문서, 긴 참고 자료 검색 | 최신 SDK, API, 변경 잦은 공식 문서 확인 |
| 장점 | 넓은 문서 집합을 두루 뒤질 수 있다 | 최신성과 출처가 더 분명하다 |
| 자주 터지는 문제 | 오래된 청크가 섞이거나 잡음이 많아진다 | 연결 실패나 소스 선정이 틀리면 바로 흔들린다 |

차이는 단순합니다. `RAG`는 넓게 찾는 데 강하고, `Context Hub`는 지금 맞는 문서를 바로 집는 데 강합니다. 최신 API처럼 시점이 중요한 영역이라면 후자가 훨씬 낫습니다.
...
에이전트가 가장 자주 망하는 순간은 계획 없이 바로 쓰기 시작할 때입니다. 그래서 계획 단계는 쓰기 권한 없는 모드로 분리하고, 그 계획을 다른 눈으로 한번 더 깨뜨린 뒤, 승인된 계획만 들고 실행 단계로 내려오는 편이 강합니다. `Planner`와 `Critic`을 같은 컨텍스트에 두지 않는 이유도 여기에 있습니다. 자기 계획을 자기가 비평하면 blind spot이 남습니다.

## **Ralph Loop: 가장 단순한 하네스**

`PROMPT.md` 하나를 루프의 중심에 두고, 실행 결과를 읽어 다시 같은 파일을 갱신하는 단순한 방식도 꽤 강력합니다. 장점은 단순성 자체입니다. 규칙이 파일 한 장에 박혀 있으니 예측 가능하고, 실수를 발견할 때마다 규칙 한 줄을 추가하면 다음 루프부터 바로 반영됩니다. 다만 검증 문이 약하면 단순함이 곧 환각 누적으로 바뀝니다.

## **암묵지를 파일로 뽑아내라**

리뷰할 때 늘 하던 말, 버그를 볼 때 반사적으로 확인하던 것, "이 경우엔 꼭 한 번 더 본다" 같은 감각은 머릿속에만 있으면 재사용되지 않습니다. 이유를 적고, 실패를 규칙으로 승화하고, 취향을 전용 에이전트나 규칙 파일로 encode하는 순간 암묵지가 하네스로 바뀝니다.
...
앞서 본 여섯 개 실행 요소를 운영 관점으로 다시 묶으면 다섯 개 기능 블록으로 정리할 수 있습니다.
