...
class BuildJaryoHtmlDeckTest(unittest.TestCase):
    ...
    def test_builder_applies_latest_slide_feedback(self) -> None:
        ...
        self.assertIn("Agent = Model + Harness", slide_017)
        self.assertIn("모델이 아닌 것은 전부 하네스", slide_017)
        self.assertIn("<section class=\"top-band\">", slide_017)
        self.assertIn("<h1 class=\"title-placeholder\">왜 Claude Code인가</h1>", slide_017)
        self.assertIn("힘이 센 말을 원하는 방향으로 몰고 멈추게 만드는 장비 전체", slide_017)
        self.assertIn("thesis-harness-equation-body", slide_017)
        self.assertIn("harness-definition-card", slide_017)
        self.assertNotIn("입니다", slide_017)

        self.assertIn("<h1 class=\"title-placeholder\">1막: Copilot과 ChatGPT, 프롬프트의 시대</h1>", slide_018)
        self.assertNotIn("<h1 class=\"title-placeholder\">1막: Copilot과 ChatGPT, 프롬프트의 시대 (2022~2024)</h1>", slide_018)
        self.assertIn("2022~2024", slide_018)
        self.assertIn("era-range", slide_018)
        self.assertIn("prompt-era-body", slide_018)

        for pattern in ["Chain-of-Thought", "ReAct", "Tree-of-Thought", "Self-Refine", "Reflexion"]:
            self.assertIn(pattern, slide_019)
        self.assertIn("prompt-pattern-lab-body", slide_019)
        self.assertIn("cot-diagram", slide_019)
        self.assertIn("cot-example-diagram", slide_019)
        self.assertIn("5개", slide_019)
        self.assertIn("2캔 × 3개", slide_019)
        self.assertIn("11개", slide_019)
        self.assertIn("pattern-example-card", slide_019)
        self.assertIn("WebShop", slide_019)
        self.assertIn("Game of 24", slide_019)
        self.assertIn("code feedback", slide_019)
        self.assertIn("reflection memory", slide_019)
        self.assertIn("react-diagram", slide_019)
        self.assertIn("tot-diagram", slide_019)
        self.assertIn("feedback-diagram", slide_019)
        self.assertNotIn("pattern-assets", slide_019)
        self.assertIn("agentic-pattern-quadrant-body", slide_020)
        self.assertIn("agentic-pattern-center", slide_020)
        self.assertIn("agentic-mini-diagram", slide_020)
        self.assertIn("Generate", slide_020)
        self.assertIn("Critique", slide_020)
        self.assertIn("Search/API", slide_020)
        self.assertIn("Reviewer", slide_020)
        self.assertNotIn("05-andrew-ng-agentic-design-patterns.png", slide_020)

        self.assertIn("<h1 class=\"title-placeholder\">프롬프트 시대의 벽</h1>", slide_021)
        self.assertIn("모델은 보지 못한 것을 알 수 없음", slide_021)
        self.assertIn("blind-prompting-matrix-body", slide_021)
        self.assertIn("2막: Cursor와 컨텍스트의 시대", slide_022)
        self.assertIn("cursor-tools-body", slide_022)
        self.assertNotIn("linear-gradient", (ROOT / "docs/03-html/shared/slide-base.css").read_text(encoding="utf-8"))
        self.assertIn("Cursor 아키텍처", slide_023)
        self.assertIn("cursor-architecture-redraw-body", slide_023)
        self.assertIn("cursor-architecture-example-canvas", slide_023)
        self.assertIn("codebase index", slide_023)
        self.assertIn("context bundle", slide_023)
        self.assertIn("architecture-side-tools", slide_023)
        self.assertNotIn("06-cursor-ai-code-editor-architecture.png", slide_023)
        for loop_step in ["gather context", "take action", "verify", "repeat"]:
            self.assertIn(loop_step, slide_024)
        self.assertIn("<h1 class=\"title-placeholder\">컨텍스트 시대의 벽</h1>", slide_024)
        self.assertIn("loop-cycle-body", slide_024)
        self.assertIn("loop-cycle-arrow", slide_024)
        self.assertNotIn("loop-repeat-arc", slide_024)
        self.assertNotIn("↺ repeat", slide_024)
        for component in ["CLAUDE.md", "Skills", "Hooks", "MCP", "Subagents", "승인/샌드박스"]:
            self.assertIn(component, slide_025)
        self.assertIn("component-tier-body", slide_025)
        for tier in ["기초", "자동화", "연결", "확장"]:
            self.assertIn(tier, slide_025)
        for decision in ["무엇을 보는지", "무엇을 할 수 있는지", "언제 멈추는지", "잘못되었을 때"]:
            self.assertIn(decision, slide_026)
        self.assertIn("harness-layered-body", slide_026)
        self.assertNotIn("harness-level", slide_026)
        ...
        self.assertRegex(css, r"\.metric-impact-body\.is-tall-reference \{[^}]*top: 112px;")
        self.assertRegex(css, r"\.metric-impact-body\.is-tall-reference \{[^}]*bottom: 54px;")
        self.assertRegex(css, r"\.metric-impact-body\.is-tall-reference \.impact-card \{[^}]*min-height: 300px;")
